...
// DeploymentService manages direct deployment
...
// NewDeploymentService creates a new deployment service with direct deployment
func NewDeploymentService(
	...
	ds.logger.Success("Deployment service started with direct deployment")
	return ds
}

// DeployDirect performs direct deployment
func (ds *DeploymentService) DeployDirect(repo models.Repository, branch string) error {
	...
	startTime := time.Now()
	ds.logger.Deploy("Starting direct deployment: %s", jobKey)

	ds.metricsMu.Lock()
	ds.totalJobs++
	ds.metricsMu.Unlock()

	if err := ds.executeDirectDeployment(repo, branch); err != nil {
		duration := time.Since(startTime)
		ds.logger.Error("Direct deployment failed after %v: %v", duration.Round(time.Second), err)
	...
	duration := time.Since(startTime)
	ds.logger.Success("Direct deployment completed: %s (took %v)", jobKey, duration.Round(time.Second))
...
// executeDirectDeployment performs the actual deployment steps
func (ds *DeploymentService) executeDirectDeployment(repo models.Repository, branch string) error {
	ds.buildMutex.Lock()
	defer ds.buildMutex.Unlock()

	repoPath := filepath.Join(ds.config.Settings.WorkDir, repo.Name, branch)

	ds.logger.Deploy("Updating repository %s:%s", repo.Name, branch)
	...
	ds.logger.Deploy("Found docker-compose file: %s", repo.ComposeFile)
